/**
 * Враг-преследователь
 * Преследует игрока при приближении, иначе блуждает
 */
...
export class EnemyChaser extends AbstractEnemy {
  ...
  public updateAI(player: Phaser.Types.Physics.Arcade.SpriteWithDynamicBody): void {
    ...
    // Если не отступаем, проверяем преследование
    // ✅ Используем chaseRadius из конфига (должен быть > 0 для преследования)
    if (this.chaseRadius > 0 && dist < this.chaseRadius) {
      // Преследуем игрока
